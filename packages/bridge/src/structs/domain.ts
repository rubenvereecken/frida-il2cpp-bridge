...
import {
    getNativeDomainGet,
    getNativeDomainGetAssemblies,
    getNativeDomainGetAssemblyFromName,
    getNativeThreadAttach,
...
export class Domain extends NativeStruct {
    ...
    get assemblies(): Assembly[] {
        let handles = readNativeList(_ => getNativeDomainGetAssemblies()(this, _));
    ...
    @memoize
    get object(): Object_ {
        return getCorlib().class('System.AppDomain').method<Object_>('get_CurrentDomain').invoke();
    }
    ...
    attach(): Thread {
        return new Thread(getNativeThreadAttach()(this));
    }

    /** Opens and loads the assembly with the given name. */
    tryAssembly(name: string): Assembly | null {
        return new Assembly(
            getNativeDomainGetAssemblyFromName()(this, Memory.allocUtf8String(name))
        ).asNullable();
    }
}

export const getDomain = memoize(() => new Domain(getNativeDomainGet()()));
