...
export class Tracer {
    ...
    /** @internal */
    #threadId: number = getMainThread().id;

    /** @internal */
    #verbose: boolean = false;

    /** @internal */
    #applier: Tracer.Apply;
...
export declare namespace Tracer {
    export type Configure = Pick<Tracer, 'thread' | 'verbose'> & Tracer.ChooseTargets;

    export type ChooseTargets = Pick<Tracer, 'domain' | 'assemblies' | 'classes' | 'methods'>;

    export type FilterAssemblies = FilterClasses & Pick<Tracer, 'filterAssemblies'>;

    export type FilterClasses = FilterMethods & Pick<Tracer, 'filterClasses'>;

    export type FilterMethods = FilterParameters & Pick<Tracer, 'filterMethods'>;

    export type FilterParameters = Pick<Tracer, 'and'> & Pick<Tracer, 'filterParameters'>;
    ...
    export type Apply = (method: Method, state: Tracer.State, threadId: number) => void;
}

/** */
export function trace(parameters: boolean = false): Tracer.Configure {
    const applier = (): Tracer.Apply => (method, state, threadId) => {
    ...
    const applierWithParameters = (): Tracer.Apply => (method, state, threadId) => {
        const paddedVirtualAddress = method.relativeVirtualAddress.toString(16).padStart(8, '0');
...
/** */
export function backtrace(mode?: Backtracer): Tracer.Configure {
    const methods = getDomain()
        .assemblies.flatMap(_ =>
        ...
        Interceptor.attach(method.virtualAddress, function () {
            if (this.threadId == threadId) {
                ...
                for (const handle of handles) {
                    if (
                        handle.compare(getModule().base) > 0 &&
