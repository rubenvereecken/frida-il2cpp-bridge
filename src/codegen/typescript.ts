namespace Il2Cpp {
    ...
    class Namer {
        ...
        static nameClass(kls: Il2Cpp.Class) {
            // From outer to inner, including this one
            const classes: Il2Cpp.Class[] = [kls, ...kls.declaringClasses].reverse();
        ...
        static nameType(type: Il2Cpp.Type) {
            return Namer.nameClass(type.class);
        }

        static name(thing: Il2Cpp.Class | Il2Cpp.Type) {
            if (thing instanceof Il2Cpp.Class) return Namer.nameClass(thing);
            if (thing instanceof Il2Cpp.Type) return Namer.nameType(thing);
        }
    }
    ...
    export class TypescriptIl2cppAnalyzer {
        analyzeField(field: Il2Cpp.Field) {}

        analyzeMethod(method: Il2Cpp.Method) {}

        analyzeClass(klass: Il2Cpp.Class) {
            ...
            const dependencies: Il2Cpp.Type[] = uniqBy(
                [
        ...
        analyzeImage(
            image: Il2Cpp.Image,
            predicate: (klass: Il2Cpp.Class) => boolean = () => true
        ) {
        ...
        analyzeNamespace(namespaceName: string, classes: ClassInfo[]) {
            ...
            const externalDependenciesByNamespace: { [key: string]: Il2Cpp.Type[] } = {};
...
${staticStr}set ${Namer.name(field.type)} ${field.name}(value: ${Namer.name(field.type)}) {
    this.${DUNDER}${OBJECT}.class.field<${Namer.name(field.type)}>('${field.name}')${bindStr}.value = value;
        ...
        writeClass({ klass }: ClassInfo): string {
            ...
            const extendsStr = klass.parent
                ? ` extends ${Namer.name(klass.parent.type.class)}`
                : ``;
